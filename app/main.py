...
async def process_request(request_id: str, pdf_path: str, db: Session):
    ...
    try:
        ...
        def extract_title_and_text():
            ...
            first_page = doc[0].get_text().strip().split('\n')[0]
            title = first_page[:100]  # Limit title length
            return title, extract_relevant_text(pdf_path, word_limit=WORD_LIMIT), extract_references_section(pdf_path)
