...
async def generate_summary(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        payload = {
            "model": os.getenv('MAIN_MODEL', 'deepseek/deepseek-v4-flash'),
            "messages": [
                {
                    "role": "user",
...
async def process_request(request_id: str, pdf_path: str, db: Session):
    ...
    try:
        ...
        if not sentences_data:
            raise Exception("Main content extraction produced no sentences; failing closed instead of completing with empty output.")
        ...
        if not references:
            raise Exception("References extraction produced no references; failing closed instead of completing with empty output.")
